...
class AirbnbTransformer:
    ...
    def transform(self) -> pd.DataFrame:
        ...
        logger.info(f"Transformation complete: {len(df)} listings, {len(df.columns)} features")
        return df
    ...
    def add_occupancy_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        calendar_agg = self.calendar.groupby('listing_id').agg({
        ...
        if 'date' in self.calendar.columns:
            ...
            summer_occ = summer.groupby('listing_id')['available'].apply(
                lambda x: 1 - x.mean()
            ).rename('summer_occupancy')
            
            # Winter occupancy (Dec-Feb)
            winter = self.calendar[self.calendar['month'].isin([12, 1, 2])]
            winter_occ = winter.groupby('listing_id')['available'].apply(
                lambda x: 1 - x.mean()
    ...
    def add_review_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        review_counts['months_active'] = (
            (pd.to_datetime(review_counts['last_review']) - 
             pd.to_datetime(review_counts['first_review'])).dt.days / 30
        ).clip(lower=1)
    ...
    def add_neighborhood_rankings(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        neighborhood_stats.columns = [
            ...
            'neighborhood_price_std', 'neighborhood_price_count',
            'neighborhood_listing_count'
        ]
        
        # Rank neighborhoods by average price
        neighborhood_stats['neighborhood_rank'] = (
            neighborhood_stats['neighborhood_avg_price'].rank(ascending=False)
        )
    ...
    def create_neighborhood_summary(self) -> pd.DataFrame:
        ...
        summary = self.listings.groupby('neighbourhood').agg({
            ...
            'number_of_reviews': 'sum' if 'number_of_reviews' in self.listings.columns else 'count',
            'availability_365': 'mean' if 'availability_365' in self.listings.columns else 'count'
        })
        
        summary.columns = [
            'listing_count', 'avg_price', 'median_price', 
            'min_price', 'max_price', 'price_std',
            'total_reviews', 'avg_availability'
        ]
    ...
    def create_room_type_summary(self) -> pd.DataFrame:
        ...
        summary = self.listings.groupby('room_type').agg({
            'id': 'count',
            'price': ['mean', 'median'],
            'number_of_reviews': 'mean' if 'number_of_reviews' in self.listings.columns else 'count'
        })
