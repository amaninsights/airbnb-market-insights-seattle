...
class AirbnbDataLoader:
    ...
    def _create_engine(self) -> Engine:
        ...
        db_path = self.config.get('database', {}).get('path', 'data/processed/airbnb_seattle.db')
    ...
    def load_listings(self, filepath: Optional[str] = None) -> pd.DataFrame:
        ...
        """
        if filepath is None:
            filename = self.config.get('data', {}).get('files', {}).get('listings', 'listings.csv')
    ...
    def load_calendar(self, filepath: Optional[str] = None) -> pd.DataFrame:
        ...
        """
        if filepath is None:
            filename = self.config.get('data', {}).get('files', {}).get('calendar', 'calendar.csv')
            filepath = self.data_dir / filename
    ...
    def load_reviews(self, filepath: Optional[str] = None) -> pd.DataFrame:
        ...
        """
        if filepath is None:
            filename = self.config.get('data', {}).get('files', {}).get('reviews', 'reviews.csv')
            filepath = self.data_dir / filename
