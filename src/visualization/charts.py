...
class AirbnbCharts:
    ...
    def plot_price_distribution(self, ax: Optional[plt.Axes] = None) -> plt.Figure:
        ...
        ax.set_title(
            "Distribution of Airbnb Prices in Seattle", fontsize=14, fontweight="bold"
        )
        ax.legend()
    ...
    def plot_neighborhood_prices(
        ...
        neighborhood_prices = self.df.groupby("neighbourhood")["price"].agg(
            ["mean", "count"]
        )
        neighborhood_prices = neighborhood_prices[
            neighborhood_prices["count"] >= 10
        ]  # Min 10 listings
        ...
        bars = ax.barh(
            neighborhood_prices.index, neighborhood_prices["mean"], color=colors
        )
        ...
        ax.set_ylabel("")
        ax.set_title(
            f"Top {top_n} Most Expensive Neighborhoods", fontsize=14, fontweight="bold"
        )
        ax.invert_yaxis()
    ...
    def plot_room_type_analysis(self, ax: Optional[plt.Axes] = None) -> plt.Figure:
        ...
        room_stats = (
            self.df.groupby("room_type")
            .agg({"id": "count", "price": "mean"})
            .rename(columns={"id": "count", "price": "avg_price"})
        )

        colors = [
            self.COLORS["airbnb_red"],
            self.COLORS["airbnb_teal"],
            self.COLORS["airbnb_orange"],
            self.COLORS["airbnb_pink"],
        ][: len(room_stats)]
        ...
        ax.set_title(
            "Listing Distribution by Room Type", fontsize=14, fontweight="bold"
        )
    ...
    def plot_availability_heatmap(self, ax: Optional[plt.Axes] = None) -> plt.Figure:
        ...
        ax.set_title(
            "Average Availability by Neighborhood & Room Type",
            fontsize=14,
            fontweight="bold",
        )
        ax.set_xlabel("Room Type", fontsize=12)
    ...
    def plot_price_map(self, ax: Optional[plt.Axes] = None) -> plt.Figure:
        ...
        ax.set_title(
            "Seattle Airbnb Listings - Price Heatmap", fontsize=14, fontweight="bold"
        )
    ...
    def plot_dashboard(self) -> plt.Figure:
        ...
        fig.suptitle(
            "Seattle Airbnb Market Dashboard", fontsize=18, fontweight="bold", y=1.02
        )
    ...
    def save_chart(
        self, fig: plt.Figure, filename: str, output_dir: str = "reports/figures"
    ) -> str:
        """
        Save figure to file.
