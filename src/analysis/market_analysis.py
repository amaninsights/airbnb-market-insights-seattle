...
class MarketAnalyzer:
    ...
    def get_market_insights(self) -> MarketInsights:
        ...
        total_hosts = self.df['host_id'].nunique() if 'host_id' in self.df.columns else 0
        ...
        if 'summer_occupancy' in self.df.columns and 'winter_occupancy' in self.df.columns:
    ...
    def analyze_pricing(self) -> Dict[str, any]:
        ...
        if 'room_type' in self.df.columns:
            analysis['by_room_type'] = self.df.groupby('room_type')['price'].agg([
                'mean', 'median', 'count'
            ]).to_dict('index')
    ...
    def _get_price_recommendation(
        ...
        if price < median * 0.8:
            return f"Consider increasing price. You're 20%+ below median (${median:.0f})"
    ...
    def correlation_analysis(self) -> pd.DataFrame:
        ...
        relevant = [c for c in numeric_cols if c not in ['id', 'host_id', 'latitude', 'longitude']]
